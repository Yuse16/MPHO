...
export type DraftOrder = { id: string; reference: string; state: 'draft' | 'quote_pending' | 'quoted' | 'pending_payment' | 'paid'; version: number; requestedDeliveryAt: string; recipient: CartRecipient; address: CartAddress; currency: 'MXN'; subtotal: Money; delivery: Money | null; service: Money | null; totalKnown: Money; totalIsFinal: boolean; availabilityStatus: 'eligible' | 'requires_review'; pendingComponents: string[]; quoteExpiresAt: string | null; review: { status: 'pending' | 'in_progress' | 'changes_required' | 'approved' | 'rejected'; publicReasonCode: string | null; publicExplanation: string | null; nextAction: string | null } | null; previousSubtotal: Money | null; difference: Money | null; priceExplanation: string | null; items: unknown[]; createdAt: string; paidAt:string|null; canStartPayment:boolean; payment:{id:string;status:'creating'|'checkout_ready'|'pending'|'approved'|'rejected'|'cancelled'|'expired'|'error'|'requires_review';amount:Money;checkoutUrl:string|null;expiresAt:string;approvedAt:string|null;canRequery:boolean}|null }
