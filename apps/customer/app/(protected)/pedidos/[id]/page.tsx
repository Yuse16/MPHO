...
export default async function OrderPage({params}:{params:Promise<{id:string}>}){
  ...
  const title=order.state==='draft'?'Pedido borrador':order.state==='quote_pending'?'Revisión en curso':'Cotización lista'
  return <main className="min-h-screen px-4 py-10"><article className="glass mx-auto max-w-2xl rounded-[2rem] p-7 sm:p-10"><p className="text-xs font-bold uppercase tracking-[.22em] text-lime">{title}</p><h1 className="mt-3 text-3xl font-extrabold">{order.reference}</h1>
    ...
    {order.state==='quoted'&&<><p className="mt-4 text-sm leading-6 text-muted-foreground">Tu cotización está lista. El pago se habilitará en la siguiente fase.</p>{order.priceExplanation&&<p className="mt-2 text-sm leading-6 text-muted-foreground">{order.priceExplanation}</p>}</>}
    ...
    {order.state==='draft'&&order.review?.status!=='changes_required'&&<OrderReviewAction orderId={order.id} version={order.version}/>}<button disabled className="mt-5 w-full rounded-full border border-white/15 px-5 py-3 font-bold opacity-50">Pago no disponible</button><div className="mt-7 rounded-2xl border border-border-soft p-4 text-xs leading-5 text-muted-foreground">No se realizó pago, reserva, asignación ni solicitud logística.</div><Link href="/" className="mt-7 inline-flex rounded-full bg-white/10 px-5 py-3 font-bold">Volver al inicio</Link></article></main>
}
